"""
aggregation.py — Layer 13 pooling + geometric and statistic features.
"""
...
def aggregate(
    ...
    """Mean-pooling токенов ответа на 13 слое."""
    h = hidden_states[13]  # (seq_len, hidden_dim)
    seq_len = attention_mask.size(0)
    
    response_mask = torch.zeros(seq_len, dtype=torch.bool, device=h.device)
    if prompt_len < seq_len:
        response_mask[prompt_len:seq_len] = attention_mask[prompt_len:].bool()
        
    h_resp = h[response_mask]
    return h_resp.mean(dim=0) if h_resp.size(0) > 0 else torch.zeros(h.size(1), device=h.device)
...
def aggregation_and_feature_extraction(
    ...
    if use_geometric:
        ...
        return torch.cat([agg, geo], dim=0)  # [896 + 6] = 902
    return agg  # [896]